...
- (void)setResidueTime:(NSNumber *)residueTime {
    objc_setAssociatedObject(self, @selector(residueTime), residueTime, OBJC_ASSOCIATION_COPY);
}

- (NSNumber *)residueTime {
    return objc_getAssociatedObject(self, @selector(residueTime));
}
...
- (void)startTimeWithDuration:(NSInteger)duration
{
    ...
    dispatch_source_set_event_handler(timer_t, ^{
        if(timeout <= 0){ //倒计时结束，关闭
            ...
            self.residueTime = [NSNumber numberWithInteger:seconds];
            if(seconds == 0){
                seconds = duration;
            }
